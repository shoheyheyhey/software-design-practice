...
class TestMovie {
    ...
    inner class CalculateCharge {
        ...
        @Test
        fun `準新作動画を割引・割増が適用されない期間内でレンタルした際のレンタル料金を検証する`() {
            val movie = Movie.movieFactory(Movie.Category.SEMI_NEW)
        ...
        @Test
        fun `準新作動画を割増が適用される期間レンタルした際のレンタル料金を検証する`() {
            val movie = Movie.movieFactory(Movie.Category.SEMI_NEW)
